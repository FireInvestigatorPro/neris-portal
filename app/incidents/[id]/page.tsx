...
function pickBackendBaseUrl() {
  // Use the same backend selection logic as dashboard to avoid env drift on Vercel.
  return (
...
export default async function IncidentDetailPage({
  ...
  const rawDeptId = searchParams?.departmentId;
  const departmentId =
  ...
  const incidentIdNum = Number(params.id);
  ...
  // 2) If direct fetch failed and we have departmentId, fallback to department incidents list and find it
  if (!incident && Number.isFinite(departmentId)) {
  ...
  // Optional: fetch department name for display (best effort)
    ...
    return (
      <div className="space-y-4">
        ...
        <p className="text-sm text-slate-300">
          We couldn’t load this incident from the backend.
        </p>
        {fetchNote ? (
          <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-4 text-xs text-slate-300">
            ...
            <div className="mt-1 text-slate-400">
              URL should include departmentId when coming from a department list, e.g. <span className="text-slate-200">?departmentId=7</span>
            </div>
          </div>
        ) : null}
  ...
  const deptLabel =
    department?.name ??
    (incident.department_id ? `Department #${incident.department_id}` : "Department");
